...
let isMediaQueryDevice;
let isMobile = isMobileDevice();

if (!isMobile){
    let mediaQuery = window.matchMedia("(max-width:700px)");
    isMediaQueryDevice = mediaQuery.matches
    ...
    for (let i = 0; i < links.length; i++) {
        ...
        if (link.href === location.href) {
            ...
            if (path[path.length - 1] === "") {
                ...
                // on mobile device, add style without animation.
                if(isMediaQueryDevice || isMobile){
                    nav[0].classList.remove('light');
                    document.body.style.overflow = ""
                } else {
                    gsap.fromTo(nav, {opacity: '1'}, {opacity: '0', onComplete() {
                            nav[0].classList.remove('light');
                            document.body.style.overflow = ""
                        }})
                    gsap.fromTo(nav, {opacity: '0'}, {opacity: '1', delay:1});
                }
            } else {
                if(isMediaQueryDevice || isMobile){
                    nav[0].classList.add('light');
